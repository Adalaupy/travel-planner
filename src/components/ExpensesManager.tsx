...
export const ExpensesManager: React.FC<Props> = ({ tripId }) => {
  ...
  useEffect(() => {
    let mounted = true
    const load = async () => {
      const [expData, travData] = await Promise.all([
    ...
    const interval = setInterval(async () => {
      const travData = await db.travelers.where('Trip_ID').equals(tripId).toArray()
  ...
  const addExpense = async () => {
    ...
    const charged = chargedTo.length ? chargedTo : travelers.map(t => t.Traveler_ID!)
  ...
  const toggleCharged = (tId: number) => {
    setChargedTo(prev => prev.includes(tId) ? prev.filter(x => x !== tId) : [...prev, tId])
  }
  ...
  return (
    <div className={styles.expensesContainer}>
      ...
      <div className={styles.addExpenseRow}>
        <input placeholder="Event/Description" value={title} onChange={e => setTitle(e.target.value)} />
        ...
        <select value={payerId || ''} onChange={e => setPayerId(Number(e.target.value))}>
          {travelers.map(t => <option key={t.Traveler_ID} value={t.Traveler_ID}>{t.name}</option>)}
        </select>
        <div className={styles.chargedToRow}>
          ...
          <div className={styles.chargedToList}>
            ...
            <button type="button" className={styles.selectAllBtn} onClick={selectAllCharged}>
              {chargedTo.length === travelers.length ? 'Unselect All' : 'Select All'}
            </button>
          </div>
        </div>
      ...
      <ul className={styles.expensesList}>
        {expenses.map(exp => {
          ...
          const charged = exp.chargedTo?.map(id => travelers.find(t => t.Traveler_ID === id)?.name).filter(Boolean).join(', ') || 'All'
          const expDate = exp.datetime ? new Date(exp.datetime).toLocaleDateString() : ''
          return (
            <li key={exp.Expense_ID} className={styles.expenseItem}>
              <div className={styles.expenseInfo}>
                <div className={styles.expenseTitle}>{exp.title}</div>
                <div className={styles.expenseMeta}>
                  ${exp.amount.toFixed(2)} • {expDate} • Paid by {payer?.name || 'Unknown'} • Charged to: {charged}
                </div>
              </div>
              <button className={styles.deleteBtn} onClick={() => removeExpense(exp.Expense_ID!)}>Remove</button>
            </li>
          )
        })}
      </ul>
      ...
      <div className={styles.balancesSection}>
        ...
        <ul className={styles.balancesList}>
          {travelers.map(t => {
            const bal = balances[t.Traveler_ID!]
            const netStr = bal.net >= 0 ? `+$${bal.net.toFixed(2)}` : `-$${Math.abs(bal.net).toFixed(2)}`
            return (
              <li key={t.Traveler_ID} className={styles.balanceItem}>
                ...
                <span className={styles.balanceDetails}>
                  Paid: ${bal.paid.toFixed(2)} | Owe: ${bal.owe.toFixed(2)} | Net: <strong>{netStr}</strong>
                </span>
              </li>
            )
          })}
          ...
          <ul className={styles.settlementsList}>
            {settlements.map((s, idx) => (
              <li key={idx} className={styles.settlementItem}>
                <strong>{s.from}</strong> pays <strong>{s.to}</strong>: <span className={styles.settlementAmount}>${s.amount.toFixed(2)}</span>
              </li>
            ))}
          </ul>
        </div>
